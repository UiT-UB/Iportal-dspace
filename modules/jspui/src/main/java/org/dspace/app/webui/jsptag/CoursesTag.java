...
public class CoursesTag extends TagSupport
{
	...
	// KMS: Changed path
    private static final String CONTROLLEDVOCABULARY_JSPTAG = "/courses/coursesTag.jsp";
	//KME
    ...
    public int doStartTag() throws JspException
    {
	    ...
	    //KMS: Get locale, try to get language. I really dont know what I am doing here but it works.
		//Gets a NullPointerException on String lang = locale.getLanguage(); when another locale than "no" is used.
		//see http://java.sun.com/webservices/docs/1.6/api/javax/servlet/jsp/jstl/fmt/LocalizationContext.html
		...
		//KMS: Change filenames and paths
        String vocabulariesPath = ConfigurationManager
                .getProperty("dspace.dir")
                + "/config/courses" + langExt + "/";
		...
		//KMS: Load the course list every time, not only once. This is because we now have two versions, norwegian and english.
        // Load vocabularies on startup
//        controlledVocabularies = (Map<String, Document>) pageContext.getServletContext().getAttribute("courses.Courses");
//        if (controlledVocabularies == null)
//        {
            controlledVocabularies = loadControlledVocabularies(vocabulariesPath);
//            pageContext.getServletContext().setAttribute("courses.Courses", controlledVocabularies);
//        }
		//KME
        ...
        try
        {
			...
			//KMS: Changed attribute name
            request.getSession().setAttribute(
                    "courses.vocabularyHTML", html);
			//KME
