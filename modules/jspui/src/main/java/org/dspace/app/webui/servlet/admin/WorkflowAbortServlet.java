...
public class WorkflowAbortServlet extends DSpaceServlet
{
	...
	//KMS: Send owned workflow item back to task list (pool)
	else if (button.equals("submit_send_back"))
        {
	    // find the workflow item
            WorkflowItem wi = WorkflowItem.find(c, UIUtil.getIntParameter(
		    request, "workflow_id"));
	    WorkflowManager.unclaim(c, wi, c.getCurrentUser());
            // now show what's left
            showWorkflows(c, request, response);
	}
	//KME
    ...
    private void showWorkflows(Context c, HttpServletRequest request,
            ...
            SQLException, AuthorizeException
    {
	//KMS: Get and sort the workflows

	// Sort by the following dc elements/qualifiers
	String[] elements = {"contributor"};
	String[] qualifiers = {"author"};

        //WorkflowItem[] w = WorkflowItem.findAll(c);
        WorkflowItem[] w = WorkflowItem.sortWorkflowItems(WorkflowItem.findAll(c), elements, qualifiers, "asc");
	//KME
