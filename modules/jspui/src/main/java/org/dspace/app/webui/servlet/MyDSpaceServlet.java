...
public class MyDSpaceServlet extends DSpaceServlet
{
	...
	//KMS: Get all workflow items you have rights to do something with, including those who are owned
        //List<WorkflowItem> pooledList = WorkflowManager.getPooledTasks(context, currentUser);
	List<WorkflowItem> pooledList = WorkflowManager.getPooledTasksAll(context, currentUser);
	//KME
        WorkflowItem[] pooled = pooledList.toArray(new WorkflowItem[pooledList.size()]);

	//KMS: Sortere array etter gitte parametere, god gammeldags selection sort. 

	String sortParam = request.getParameter("sort");
	
	// The same sort parameters are used for both pooled and owned.
	// If owned should have a different sorting than pooled, just add two arrays (elementsOwned, qualifiersOwned) with the desired metadata,
	// and put these two arrays in the last line of the if-clause instead of elementsPooled and qualifiersPooled.
	// Default sort
	if(sortParam == null){
	    String[] elementsPooled = {"subject", "subject", "contributor"};
	    String[] qualifiersPooled = {"departmentID", "courseID", "author"};
	    pooled = WorkflowItem.sortWorkflowItems((WorkflowItem[]) pooledList.toArray(pooled), elementsPooled, qualifiersPooled, "asc");
	    owned = WorkflowItem.sortWorkflowItems((WorkflowItem[]) ownedList.toArray(owned), elementsPooled, qualifiersPooled, "asc");
	}
	// Submission OK
	else if(sortParam.equals("subok")){
	    String[] elementsPooled = {"description", "description", "subject", "contributor"};
	    String[] qualifiersPooled = {"checked", "print", "courseID", "author"};
	    pooled = WorkflowItem.sortWorkflowItems((WorkflowItem[]) pooledList.toArray(pooled), elementsPooled, qualifiersPooled, "asc");
	    owned = WorkflowItem.sortWorkflowItems((WorkflowItem[]) ownedList.toArray(owned), elementsPooled, qualifiersPooled, "asc");
	}
	// Print 
	else if(sortParam.equals("print")){
	    String[] elementsPooled = {"description", "subject", "contributor"};
	    String[] qualifiersPooled = {"print", "courseID", "author"};
	    pooled = WorkflowItem.sortWorkflowItems((WorkflowItem[]) pooledList.toArray(pooled), elementsPooled, qualifiersPooled, "asc");
	    owned = WorkflowItem.sortWorkflowItems((WorkflowItem[]) ownedList.toArray(owned), elementsPooled, qualifiersPooled, "asc");
	}
	// Title
	else if(sortParam.equals("title")){
	    String[] elementsPooled = {"title"};
	    String[] qualifiersPooled = {null};
	    pooled = WorkflowItem.sortWorkflowItems((WorkflowItem[]) pooledList.toArray(pooled), elementsPooled, qualifiersPooled, "asc");
	    owned = WorkflowItem.sortWorkflowItems((WorkflowItem[]) ownedList.toArray(owned), elementsPooled, qualifiersPooled, "asc");
	}
	// Course ID
	else if(sortParam.equals("course")){
	    String[] elementsPooled = {"subject", "contributor"};
	    String[] qualifiersPooled = {"courseID", "author"};
	    pooled = WorkflowItem.sortWorkflowItems((WorkflowItem[]) pooledList.toArray(pooled), elementsPooled, qualifiersPooled, "asc");
	    owned = WorkflowItem.sortWorkflowItems((WorkflowItem[]) ownedList.toArray(owned), elementsPooled, qualifiersPooled, "asc");
	}
	// Author
	else if(sortParam.equals("author")){
	    String[] elementsPooled = {"contributor"};
	    String[] qualifiersPooled = {"author"};
	    pooled = WorkflowItem.sortWorkflowItems((WorkflowItem[]) pooledList.toArray(pooled), elementsPooled, qualifiersPooled, "asc");
	    owned = WorkflowItem.sortWorkflowItems((WorkflowItem[]) ownedList.toArray(owned), elementsPooled, qualifiersPooled, "asc");
	}
	// Print examination
	else if(sortParam.equals("printex")){
	    String[] elementsPooled = {"description", "subject", "contributor"};
	    String[] qualifiersPooled = {"printexamination", "courseID", "author"};
	    pooled = WorkflowItem.sortWorkflowItems((WorkflowItem[]) pooledList.toArray(pooled), elementsPooled, qualifiersPooled, "asc");
	    owned = WorkflowItem.sortWorkflowItems((WorkflowItem[]) ownedList.toArray(owned), elementsPooled, qualifiersPooled, "asc");
	}
	// Date submitted (date.issued for master)
	else if(sortParam.equals("submitdate")){
	    String[] elementsPooled = {"date"};
	    String[] qualifiersPooled = {"issued"};
	    pooled = WorkflowItem.sortWorkflowItems((WorkflowItem[]) pooledList.toArray(pooled), elementsPooled, qualifiersPooled, "desc");
	    owned = WorkflowItem.sortWorkflowItems((WorkflowItem[]) ownedList.toArray(owned), elementsPooled, qualifiersPooled, "desc");
	}
	// Default sort if sortParam is set to something unknown
	else {
	    String[] elementsPooled = {"subject", "subject", "contributor"};
	    String[] qualifiersPooled = {"departmentID", "courseID", "author"};
	    pooled = WorkflowItem.sortWorkflowItems((WorkflowItem[]) pooledList.toArray(pooled), elementsPooled, qualifiersPooled, "asc");
