...
public class ItemTag extends TagSupport
{
    ...
    private void render() throws IOException, SQLException, DCInputsReaderException
    {
	    ...
	    //KMS: Special case: Course code as faculty name when doctoral thesis
		...
		//KME
			...
			String value = values[j].value;
                        if (value.startsWith("DOKTOR")){

			    try {
				out.print(I18nUtil.getMessage("ub.jsp.submit.select-collection." + value, UIUtil.getSessionLocale(request)));
			    }
			    catch(MissingResourceException e){
				out.print(LocaleSupport.getLocalizedMessage(pageContext, "ub.jsp.submit.select-collection." + value));
			    }
			}
			else {
			    out.print(value);
		    ...
		    //KME
			//KMS: Remove the browse link on authors
