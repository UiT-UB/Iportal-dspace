...
<%!
    ...
    String doControlledVocabulary(String fieldName, PageContext pageContext, String vocabulary, boolean readonly)
    {
        ...
        if (enabled && useWithCurrentField && !readonly)
        {
					    ...
					    fieldName2 = fieldName.substring(0, fieldName.length() - 1) + ((Integer.parseInt(fieldName.substring(fieldName.length() - 1, fieldName.length()))) + 1);

                        link = 
                        "<a href='javascript:void(null);' onclick='javascript:popUp(\"" +
                                contextPath + "/controlledvocabulary/controlledvocabulary.jsp?ID1=" +
                                fieldName1 + "&amp;ID2=" + fieldName2 + "&amp;vocabulary=" + vocabulary + "\")'>" +
    ...
    void doPersonalName(javax.servlet.jsp.JspWriter out, Item item,
      String fieldName, String schema, String element, String qualifier, boolean repeatable, boolean required,
			boolean readonly, int fieldCountIncr, String label, PageContext pageContext, int collectionID, EPerson user)
      throws java.io.IOException
    {
      ...
      for (int i = 0; i < fieldCount; i++)
      {
	     ...
	     // If no author name is already stored in the database, use current submitter's name 
	     if (element.equals("contributor") && qualifier.equals("author") && i == 0){
		 dpn = new org.dspace.content.DCPersonName(user.getLastName(), user.getFirstName());
	     }
	     else {
		 dpn = new org.dspace.content.DCPersonName();
	     }
...
//          .append(">(No month)</option>");
    ...
    //KM: This method is modified to work with the modifications of controlled vocabulary.
      ...
      throws java.io.IOException
    {
      ...
      sb.append("<div class=\"row\"><label class=\"col-md-2"+ (required?" label-required":"") + " margin-top-19px\">")
        .append(label)
        .append("</label>");
      sb.append("<div class=\"col-md-10\">");
      for (int i = 0; i < fieldCount; i++)
      {
		 ...
		 sb.append("<span class=\"col-md-4\">")
          ...
          if (!readonly)
          {
                       sb.append("<button class=\"btn btn-danger col-md-2 margin-top-19px\" name=\"submit_")
                             .append(fieldName)
                             .append("_remove_")
                             .append(i)
                             .append("\" value=\"")
         ...
         //        }
				 ...
				 sb.append("<span class=\"col-md-4\">")
                   ...
                   {
                     sb.append(" <button class=\"btn btn-default col-md-2 margin-top-19px\" name=\"submit_")
                       .append(fieldName)
                       .append("_add\" value=\"")
                       .append(LocaleSupport.getLocalizedMessage(pageContext, "jsp.submit.edit-metadata.button.add"))
                       .append("\"><span class=\"glyphicon glyphicon-plus\"></span>&nbsp;&nbsp;"+LocaleSupport.getLocalizedMessage(pageContext, "jsp.submit.edit-metadata.button.add")+"</button>\n");
...
<%
	...
	{
%>
	    <dspace:popup page="<%= LocaleSupport.getLocalizedMessage(pageContext, \"ub.help.master-path\") + \"#describe2\"%>"><fmt:message key="jsp.morehelp"/></dspace:popup>
<%
	}
%>
...
<%
	...
	{
%>
            <dspace:popup page="<%= LocaleSupport.getLocalizedMessage(pageContext, \"ub.help.master-path\") + \"#describe3\"%>"><fmt:message key="jsp.morehelp"/></dspace:popup>
<%
	}
%>
     ...
     for (int z = 0; z < inputs.length; z++)
     {
       ...
       if (inputType.equals("name"))
       {
           doPersonalName(out, item, fieldName, dcSchema, dcElement, dcQualifier,
			  repeatable, required, readonly, fieldCountIncr, label, pageContext, collectionID, user);
       }
