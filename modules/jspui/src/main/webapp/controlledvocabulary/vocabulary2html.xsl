...
<!--<xsl:variable name="allowMultipleSelection">no</xsl:variable> -->
		...
		<li>
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					...
					<xsl:choose>
						<xsl:when test="@id='VDP'">
							<span class="value">
								<xsl:value-of select="@label"/>
							</span>
						</xsl:when>
						<xsl:when test="@id='000'">
							<span class="value">
								<xsl:value-of select="@label"/>
							</span>
						</xsl:when>
						...
						<xsl:when test="@id='700'">
							<span class="value">
								<xsl:value-of select="@label"/>
							</span>
						</xsl:when>		
						<xsl:when test="@id='900'">
							<span class="value">
								<xsl:value-of select="@label"/>
							</span>
						</xsl:when>	
						<xsl:otherwise>
						  ...
						  <xsl:variable name="id_node">
						    <xsl:value-of select="@id"/>
						  </xsl:variable>
						  
						  <a name="{$id_node}" class="value" onclick="javascript: i(this);" href="javascript:void(null);">
						    <xsl:value-of select="@label"/>
						  </a>
						</xsl:otherwise>
					</xsl:choose>
	...
	<xsl:template name="getNodePath">
		...
		<!-- Search only for the id, not the full string. -->
		<xsl:value-of select="@id"/>
	</xsl:template>
	<!-- ************************************ -->
</xsl:stylesheet>
