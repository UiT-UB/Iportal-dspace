  ...
  <script type="text/javascript">
	<%
		if(request.getParameter("ID1")!=null) {
			session.setAttribute("controlledvocabulary.ID1", request.getParameter("ID1"));
		}
		if(request.getParameter("ID2")!=null) {
			session.setAttribute("controlledvocabulary.ID2", request.getParameter("ID2"));
		}
	%>
	...
	function sendBackToParentWindow(node) {
		var resultPath = "";
		var resultPathOtherLang = "";

		var firstNode = 1;
		var pathSeparator = "::";
		
	        var nodesLang = document.getElementsByName(node.name);
	        var nodeOtherLang = nodesLang[1];
		...
		var firstNode = 1;
                ...
                while(nodeOtherLang != null) {
			if(firstNode == 1) {
				resultPathOtherLang = getTextValue(nodeOtherLang);
				firstNode = 0;
			} else {
				resultPathOtherLang = getTextValue(nodeOtherLang) + pathSeparator + resultPathOtherLang;
			}
			nodeOtherLang = getParentTextNode(nodeOtherLang);
		}

		window.opener.document.edit_metadata.<%=session.getAttribute("controlledvocabulary.ID1")%>.value= resultPath;
		window.opener.document.edit_metadata.<%=session.getAttribute("controlledvocabulary.ID2")%>.value= resultPathOtherLang;
...
<div class="submitFormHelpControlledVocabularies">
	<dspace:popup page="/help/index.html#controlledvocabulary"><fmt:message key="jsp.controlledvocabulary.controlledvocabulary.help-link"/></dspace:popup>
</div>
</p>

<%-- Remove the parameter 'vocabulary="<%= vocabulary %>"' from the controlledvocabulary tag. Makes it possible to have more than one vocabulary. --%>
<dspace:controlledvocabulary filter="<%= filter %>" vocabulary="<%= vocabulary %>" /> 

<%
//KM: NB: Hardcoded vocabulary names. Should be fixed to dynamically select vocabularies
String vocabularyOther = "nsi";
if(vocabulary.equals("nsi")){
    vocabularyOther = "nsi_no";
}
%>

<div style="display: none;">
<dspace:controlledvocabulary filter="<%= filter %>" vocabulary="<%= vocabularyOther %>" /> 
</div>
