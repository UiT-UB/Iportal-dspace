...
public class Util {
    ...
    //KMS: Check if the item is a doctoral thesis
    /**
     * A metadata field (dc.type) is used to decide if the submission in question
     * is a doctoral thesis or not. These items will be treated differently 
     * from other documents in the workspace and workflows.
     * This method assumes that the item is submitted to a collection that has a
     * predefined dc.type-field 'Doctoral thesis'.
     *
     * @param item
     *            the item in question
     *
     * @return true if item is a doctoral thesis, false otherwise
     */
    public static boolean isDr(Item item){
	
	DCValue[] types = item.getMetadata("dc", "type", null, Item.ANY);

	for(int i=0; i<types.length; i++){
	    if(types[i].value.equals("Doctoral thesis")){
		return true;
	    }
	}
	return false;
    ...
    /**
     * Check if this is a submission that the printing service wants to see
     * The in-house printing service only wants submissions in their list which are ready for print
     *
     * @param item
     *            the item in question
     * @param doCheck
     *            whether to do the check or not
     *
     * @return true if item is a doctoral thesis, false otherwise
     */
    public static boolean isPrint(Item item, boolean doCheck){
	
	// If print view is not activated, always show the submission 
	if(!doCheck){
	    return true;
	}

	// If print view is activated, show only submissions the printing service wants to see
	else{
	    DCValue[] checked = item.getMetadata("dc", "description", "checked", Item.ANY);
	    DCValue[] print = item.getMetadata("dc", "description", "print", Item.ANY);
	    
	    if(checked.length > 0 && print.length > 0){
		if(checked[0].value.equals("Ja") && (!print[0].value.equals("I produksjon") && !print[0].value.equals("Mottatt fra trykkeri"))){
		    return true;
		}
	    }
	    return false;
