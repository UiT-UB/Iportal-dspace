...
public class SelectCollectionStep extends AbstractProcessingStep
{
            ...
            AuthorizeException
    {
        ...
        else
        {
	    	...
	    	//KMS: Store the course ID and department ID in a metadata field (dc.subject.courseID, dc.subject.departmentID)
	    
	    	subInfo.getSubmissionItem().getItem()
				.addDC("subject", "courseID", null, request.getParameter("courseID"));
	    	subInfo.getSubmissionItem().getItem()
				.addDC("subject", "departmentID", null, request.getParameter("departmentID"));


	    	// All doctoral theses will be exported to Munin
	    	if(Util.isDr(subInfo.getSubmissionItem().getItem())){
				subInfo.getSubmissionItem().getItem()
		    		.addDC("description", "publish", null, "N\u00E5");
	    	}
	    	// Include department name for master theses
	    	else {
				subInfo.getSubmissionItem().getItem()
		    		.addDC("description", "department", null, request.getParameter("departmentName"));
	    	}
