...
//KMS: Import WorkflowItem, WorkflowManager
...
public class InitialQuestionsStep extends AbstractProcessingStep
{
    ...
    //KMS: Need extra flag for pruning av fields in doctoral submission
    protected boolean willRemoveDoctoral = false;
    //KME
            ...
            AuthorizeException
    {
		...
		//KMS: We use multipleTitles as an identifier for wanting to publish a document later, 
		//     and publishedBefore as an identifier for never wanting to publish a document (see more comments in initial-questions.jsp).
		...
		boolean publishedBefore = false;
	
		// From initial-questions
		if(request.getParameter("publish") != null){
	    	if(request.getParameter("publish").equals("publish_later")){
				multipleTitles = true;
	    	}
	   		if(request.getParameter("publish").equals("publish_never")){
				publishedBefore = true;
	    	}
		}
		// From verify-prune
		else{
	    	multipleTitles = Util.getBoolParameter(request, "multiple_titles");
	    	publishedBefore = Util.getBoolParameter(request, "published_before");
		}

		// Set multiple_files = true. We want this to be the default value. Remove the question in the jsp too.
        ...
        // otherwise, check if pruning is necessary
        {
            ...
            if (subInfo.getSubmissionItem() != null)
            {
                ...
                if (!multipleTitles)
                {
					//KMS: Check if the explanation and/or embargo period field has a value. Has to be removed if the submitter changes opinion on the publishing question
                    //DCValue[] altTitles = subInfo.getSubmissionItem().getItem()
                    //        .getDC("title", "alternative", Item.ANY);

                    //willRemoveTitles = altTitles.length > 0;

					DCValue[] explanation = subInfo.getSubmissionItem().getItem().getDC("description", "explanation", Item.ANY);
					DCValue[] embargoPeriod = subInfo.getSubmissionItem().getItem().getDC("description", "embargoyears", Item.ANY);
		
					willRemoveTitles = (explanation.length > 0) || (embargoPeriod.length > 0);
					//KME
                }

                if (!publishedBefore)
                {
					//KMS: Check if the argument field and the doctoral thesis publishing fields has a value. Has to be removed if the submitter changes opinion on the publishing question
					...
					DCValue[] argument = subInfo.getSubmissionItem().getItem().getDC("description", "argument", Item.ANY);
	
					willRemoveDate = argument.length > 0;
					//KME
                }
				...
				//KMS: Special case for doctoral submission, some fields always have to removed when the publishing question is changed
            	if (publishedBefore)
            	{
					DCValue[] condition = subInfo.getSubmissionItem().getItem().getDC("description", "condition", Item.ANY);
					DCValue[] consent = subInfo.getSubmissionItem().getItem().getDC("description", "consent", Item.ANY);
					DCValue[] consentextended = subInfo.getSubmissionItem().getItem().getDC("description", "consentextended", Item.ANY);
					DCValue[] unpublished = subInfo.getSubmissionItem().getItem().getDC("description", "unpublished", Item.ANY);
		
					willRemoveDoctoral = (condition.length > 0) || (consent.length > 0) || (consentextended.length > 0) || (unpublished.length > 0);
	    		}
	    		//KME
            }
			...
			//KMS: Added doctoral flag
            //if (willRemoveTitles || willRemoveDate || willRemoveFiles)
			if (willRemoveTitles || willRemoveDate || willRemoveFiles || willRemoveDoctoral)
			//KME
            {
				...
				//KMS: Added doctoral flag
                request.setAttribute("will.remove.doctoral", new Boolean(willRemoveDoctoral));
				//KME				
		...
		//KMS: Must update both workflow item and workspace item if this is a doctoral thesis that is in workflow mode
        //subInfo.getSubmissionItem().setMultipleTitles(multipleTitles);
        //subInfo.getSubmissionItem().setPublishedBefore(publishedBefore);

		WorkspaceItem wsItem = WorkflowManager.getWorkspaceItem(context, subInfo.getSubmissionItem().getItem());
 		WorkflowItem wfItem = WorkflowManager.getWorkflowItem(context, subInfo.getSubmissionItem().getItem());

		if(wfItem != null && wsItem != null){

	    	wsItem.setPublishedBefore(publishedBefore);
	    	wfItem.setPublishedBefore(publishedBefore);
	    	wsItem.setMultipleTitles(multipleTitles);
	    	wfItem.setMultipleTitles(multipleTitles);
	    	log.info(LogManager.getHeader(context, "set_parameters", "0"));
		}
		else {
	    	subInfo.getSubmissionItem().setMultipleTitles(multipleTitles);
	    	subInfo.getSubmissionItem().setPublishedBefore(publishedBefore);
	    	log.info(LogManager.getHeader(context, "set_parameters", "1"));
		}
		//KME
		...
		//KMS: Comment out this code. We do not have any indication of previous publishing since we are using publishedBefore for other things
        //if(!publishedBefore)
        //{
        //    DCValue[] dateIssued = subInfo.getSubmissionItem().getItem()
        //                    .getDC("date", "issued", Item.ANY);
		...
		if(wfItem != null && wsItem != null){
	    	wsItem.update();
	    	wfItem.update();
	    	log.info(LogManager.getHeader(context, "set_parameters", "2"));
		}
		else {
	    	subInfo.getSubmissionItem().update();
	    	log.info(LogManager.getHeader(context, "set_parameters", "3"));
		}
		//KME
        context.commit();
            ...
            AuthorizeException
    {
        ...
        {
			//KMS: Delete the explanation and embargo period for not publishing, instead of the original fields connected to multipleTitles
            //item.clearDC("title", "alternative", Item.ANY);
			item.clearDC("description", "explanation", Item.ANY);
		    item.clearDC("description", "embargoyears", Item.ANY);
			//KME
        }

        if (!publishedBefore && subInfo.getSubmissionItem().isPublishedBefore())
        {
			//KMS: Delete the argument for not publishing, instead of the original fields connected to published_before
            //item.clearDC("date", "issued", Item.ANY);
            //item.clearDC("identifier", "citation", Item.ANY);
            //item.clearDC("publisher", null, Item.ANY);
			item.clearDC("description", "argument", Item.ANY);
			//KME
        }

		//KMS: Delete the fields describing publishing issues for doctoral theses
        if (publishedBefore == true && !subInfo.getSubmissionItem().isPublishedBefore())
        {
	    	item.clearDC("description", "condition", Item.ANY);
	    	item.clearDC("description", "consent", Item.ANY);
	    	item.clearDC("description", "consentextended", Item.ANY);
        }
		//KME
